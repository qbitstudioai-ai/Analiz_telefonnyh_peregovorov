...
begin
  if not exists (
    ...
    raise exception 'DB-05 verification failed: schema shablon_analiz_telefonnyh_peregovorov does not exist';
  end if;
  ...
  if not exists (
    ...
    where con.conrelid = 'shablon_analiz_telefonnyh_peregovorov.operation_attempts'::regclass
  ...
  if not exists (
    select 1
    from pg_constraint con
    where con.conrelid = 'shablon_analiz_telefonnyh_peregovorov.role_assignment_versions'::regclass
  ...
  into v_definition
  from pg_constraint con
  where con.conrelid = 'shablon_analiz_telefonnyh_peregovorov.outgoing_actions'::regclass
  ...
  into v_definition
  from pg_constraint con
  where con.conrelid = 'shablon_analiz_telefonnyh_peregovorov.delivery_attempts'::regclass
  ...
  into v_definition
  from pg_constraint con
  where con.conrelid = 'shablon_analiz_telefonnyh_peregovorov.callback_links'::regclass
  ...
  into v_definition
  from pg_constraint con
  where con.conrelid = 'shablon_analiz_telefonnyh_peregovorov.audit_events'::regclass
  ...
  into v_definition
  from pg_constraint con
  where con.conrelid = 'shablon_analiz_telefonnyh_peregovorov.corrections'::regclass
  ...
  begin
    insert into shablon_analiz_telefonnyh_peregovorov.outgoing_actions (
      call_id,
      analysis_id,
      logical_action_key,
      purpose_code,
  ...
  begin
    update shablon_analiz_telefonnyh_peregovorov.audit_events
  ...
  begin
    delete from shablon_analiz_telefonnyh_peregovorov.audit_events
    where audit_event_id = v_audit_id;
