...
-- APPROVED WORKING CONTOUR. Depends on DB-01 and DB-02; scope is limited to schema shablon_analiz_telefonnyh_peregovorov.
--
-- Key invariants:
...
begin
  if not exists (
    ...
    raise exception 'DB-03 requires schema shablon_analiz_telefonnyh_peregovorov';
  end if;
...
create type shablon_analiz_telefonnyh_peregovorov.config_version_state as enum (
  'draft',
  'ready',
  'active',
  'superseded',
...
create type shablon_analiz_telefonnyh_peregovorov.knowledge_editorial_state as enum (
  'draft',
  'ready',
  'archived'
);

create type shablon_analiz_telefonnyh_peregovorov.embedding_state as enum (
  'pending',
  'ready',
  'failed',
  'invalidated'
...
create type shablon_analiz_telefonnyh_peregovorov.knowledge_publication_state as enum (
  'draft',
  'ready',
  'published',
  'superseded',
...
create function shablon_analiz_telefonnyh_peregovorov.guard_config_initial_state()
returns trigger
language plpgsql
...
create function shablon_analiz_telefonnyh_peregovorov.guard_config_semantic_update()
returns trigger
language plpgsql
set search_path = pg_catalog
as $function$
...
-- Prompt versions.
create table shablon_analiz_telefonnyh_peregovorov.prompt_versions (
  ...
  config_state shablon_analiz_telefonnyh_peregovorov.config_version_state not null default 'draft',
  ...
  constraint prompt_versions_predecessor_same_family
    ...
    references shablon_analiz_telefonnyh_peregovorov.prompt_versions(prompt_version_id, family_ref)
    on delete restrict
);

create unique index uq_prompt_versions_family_version
  on shablon_analiz_telefonnyh_peregovorov.prompt_versions (family_ref, version_no);

create unique index uq_prompt_versions_one_active
  on shablon_analiz_telefonnyh_peregovorov.prompt_versions (family_ref)
  where config_state = 'active';

create trigger trg_prompt_versions_guard_initial_state
before insert on shablon_analiz_telefonnyh_peregovorov.prompt_versions
for each row execute function shablon_analiz_telefonnyh_peregovorov.guard_config_initial_state();

create trigger trg_prompt_versions_guard_semantic_update
before update on shablon_analiz_telefonnyh_peregovorov.prompt_versions
for each row execute function shablon_analiz_telefonnyh_peregovorov.guard_config_semantic_update();

-- Methodology versions.
create table shablon_analiz_telefonnyh_peregovorov.methodology_versions (
  ...
  constraint methodology_versions_predecessor_same_family
    ...
    references shablon_analiz_telefonnyh_peregovorov.methodology_versions(methodology_version_id, family_ref)
    on delete restrict
);

create unique index uq_methodology_versions_family_version
  on shablon_analiz_telefonnyh_peregovorov.methodology_versions (family_ref, version_no);

create unique index uq_methodology_versions_one_active
  on shablon_analiz_telefonnyh_peregovorov.methodology_versions (family_ref)
  where config_state = 'active';

create trigger trg_methodology_versions_guard_initial_state
before insert on shablon_analiz_telefonnyh_peregovorov.methodology_versions
for each row execute function shablon_analiz_telefonnyh_peregovorov.guard_config_initial_state();

create trigger trg_methodology_versions_guard_semantic_update
before update on shablon_analiz_telefonnyh_peregovorov.methodology_versions
for each row execute function shablon_analiz_telefonnyh_peregovorov.guard_config_semantic_update();

create table shablon_analiz_telefonnyh_peregovorov.methodology_criteria (
...
create table shablon_analiz_telefonnyh_peregovorov.methodology_stages (
  methodology_version_id uuid not null references shablon_analiz_telefonnyh_peregovorov.methodology_versions(methodology_version_id) on delete restrict,
...
create function shablon_analiz_telefonnyh_peregovorov.guard_methodology_child_mutation()
returns trigger
language plpgsql
...
declare
  ...
  v_state shablon_analiz_telefonnyh_peregovorov.config_version_state;
begin
...
before insert or update or delete on shablon_analiz_telefonnyh_peregovorov.methodology_criteria
for each row execute function shablon_analiz_telefonnyh_peregovorov.guard_methodology_child_mutation();

create trigger trg_methodology_stages_guard
before insert or update or delete on shablon_analiz_telefonnyh_peregovorov.methodology_stages
for each row execute function shablon_analiz_telefonnyh_peregovorov.guard_methodology_child_mutation();

-- Filter rule versions.
create table shablon_analiz_telefonnyh_peregovorov.filter_rule_versions (
  ...
  constraint filter_rule_versions_predecessor_same_family
    ...
    references shablon_analiz_telefonnyh_peregovorov.filter_rule_versions(filter_rule_version_ref, family_ref)
    on delete restrict
);

create unique index uq_filter_rule_versions_family_version
  on shablon_analiz_telefonnyh_peregovorov.filter_rule_versions (family_ref, version_no);

create unique index uq_filter_rule_versions_one_active
  on shablon_analiz_telefonnyh_peregovorov.filter_rule_versions (family_ref)
  where config_state = 'active';

create trigger trg_filter_rule_versions_guard_initial_state
before insert on shablon_analiz_telefonnyh_peregovorov.filter_rule_versions
for each row execute function shablon_analiz_telefonnyh_peregovorov.guard_config_initial_state();

create trigger trg_filter_rule_versions_guard_semantic_update
before update on shablon_analiz_telefonnyh_peregovorov.filter_rule_versions
for each row execute function shablon_analiz_telefonnyh_peregovorov.guard_config_semantic_update();
...
alter table shablon_analiz_telefonnyh_peregovorov.filter_decisions
  ...
  references shablon_analiz_telefonnyh_peregovorov.filter_rule_versions(filter_rule_version_ref)
  on delete restrict;

-- Canonical knowledge document.
create table shablon_analiz_telefonnyh_peregovorov.knowledge_documents (
...
create table shablon_analiz_telefonnyh_peregovorov.knowledge_document_versions (
  document_version_id uuid primary key default gen_random_uuid(),
  document_id uuid not null references shablon_analiz_telefonnyh_peregovorov.knowledge_documents(document_id) on delete restrict,
  version_no integer not null,
  ...
  editorial_state shablon_analiz_telefonnyh_peregovorov.knowledge_editorial_state not null default 'draft',
  validation_status shablon_analiz_telefonnyh_peregovorov.validation_status not null default 'pending',
  ...
  constraint knowledge_document_versions_predecessor_same_document
    ...
    references shablon_analiz_telefonnyh_peregovorov.knowledge_document_versions(document_version_id, document_id)
    on delete restrict
);

create unique index uq_knowledge_document_versions_number
  on shablon_analiz_telefonnyh_peregovorov.knowledge_document_versions (document_id, version_no);

create unique index uq_knowledge_document_versions_one_current
  on shablon_analiz_telefonnyh_peregovorov.knowledge_document_versions (document_id)
  where version_state = 'current';

-- Chunk/fragment version.
create table shablon_analiz_telefonnyh_peregovorov.knowledge_fragments (
  ...
  constraint knowledge_fragments_predecessor_same_family
    ...
    references shablon_analiz_telefonnyh_peregovorov.knowledge_fragments(fragment_id, document_id, fragment_family_key)
    on delete restrict
);

create unique index uq_knowledge_fragments_version
  on shablon_analiz_telefonnyh_peregovorov.knowledge_fragments (
    document_version_id,
    fragment_family_key,
    fragment_version_no
  );
...
create unique index uq_knowledge_fragments_one_current
  on shablon_analiz_telefonnyh_peregovorov.knowledge_fragments (document_id, fragment_family_key)
  where version_state = 'current';
...
create table shablon_analiz_telefonnyh_peregovorov.knowledge_embeddings (
  embedding_id uuid primary key default gen_random_uuid(),
  fragment_id uuid not null references shablon_analiz_telefonnyh_peregovorov.knowledge_fragments(fragment_id) on delete restrict,
  version_no integer not null,
  ...
  embedding_state shablon_analiz_telefonnyh_peregovorov.embedding_state not null default 'pending',
  validation_status shablon_analiz_telefonnyh_peregovorov.validation_status not null default 'pending',
  version_state shablon_analiz_telefonnyh_peregovorov.artifact_version_state not null default 'candidate',
  ...
  constraint knowledge_embeddings_predecessor_same_family
    ...
    references shablon_analiz_telefonnyh_peregovorov.knowledge_embeddings(
      embedding_id,
      fragment_id,
      provider,
      model_name,
...
create unique index uq_knowledge_embeddings_version
  on shablon_analiz_telefonnyh_peregovorov.knowledge_embeddings (
    fragment_id,
    provider,
    model_name,
    config_version,
...
create table shablon_analiz_telefonnyh_peregovorov.knowledge_publications (
  publication_id uuid primary key default gen_random_uuid(),
  family_ref text not null,
  version_no integer not null,
  predecessor_publication_id uuid,
  publication_state shablon_analiz_telefonnyh_peregovorov.knowledge_publication_state not null default 'draft',
  validation_status shablon_analiz_telefonnyh_peregovorov.validation_status not null default 'pending',
  ...
  constraint knowledge_publications_predecessor_same_family
    ...
    references shablon_analiz_telefonnyh_peregovorov.knowledge_publications(publication_id, family_ref)
    on delete restrict
);

create unique index uq_knowledge_publications_family_version
  on shablon_analiz_telefonnyh_peregovorov.knowledge_publications (family_ref, version_no);

create unique index uq_knowledge_publications_one_current
  on shablon_analiz_telefonnyh_peregovorov.knowledge_publications (family_ref)
  where is_current;

create table shablon_analiz_telefonnyh_peregovorov.knowledge_publication_documents (
  publication_id uuid not null references shablon_analiz_telefonnyh_peregovorov.knowledge_publications(publication_id) on delete restrict,
  document_version_id uuid not null,
  document_id uuid not null,
...
create table shablon_analiz_telefonnyh_peregovorov.knowledge_publication_fragments (
  ...
  constraint knowledge_publication_fragments_document_membership
    ...
    references shablon_analiz_telefonnyh_peregovorov.knowledge_publication_documents(publication_id, document_version_id)
    on delete restrict,
  constraint knowledge_publication_fragments_exact_fragment
    foreign key (fragment_id, document_version_id, document_id)
    references shablon_analiz_telefonnyh_peregovorov.knowledge_fragments(fragment_id, document_version_id, document_id)
    on delete restrict,
  constraint knowledge_publication_fragments_exact_embedding
    foreign key (embedding_id, fragment_id)
    references shablon_analiz_telefonnyh_peregovorov.knowledge_embeddings(embedding_id, fragment_id)
    on delete restrict
);

create table shablon_analiz_telefonnyh_peregovorov.knowledge_publication_fragment_products (
  publication_id uuid not null,
  fragment_id uuid not null,
...
create function shablon_analiz_telefonnyh_peregovorov.guard_knowledge_publication_membership()
returns trigger
language plpgsql
set search_path = pg_catalog, shablon_analiz_telefonnyh_peregovorov
as $function$
declare
  ...
  v_state shablon_analiz_telefonnyh_peregovorov.knowledge_publication_state;
begin
  if tg_op = 'DELETE' then
...
before insert or update or delete on shablon_analiz_telefonnyh_peregovorov.knowledge_publication_documents
for each row execute function shablon_analiz_telefonnyh_peregovorov.guard_knowledge_publication_membership();

create trigger trg_knowledge_publication_fragments_guard
before insert or update or delete on shablon_analiz_telefonnyh_peregovorov.knowledge_publication_fragments
for each row execute function shablon_analiz_telefonnyh_peregovorov.guard_knowledge_publication_membership();

create trigger trg_knowledge_publication_products_guard
before insert or update or delete on shablon_analiz_telefonnyh_peregovorov.knowledge_publication_fragment_products
for each row execute function shablon_analiz_telefonnyh_peregovorov.guard_knowledge_publication_membership();

-- Publication row becomes immutable by meaning after publication.
create function shablon_analiz_telefonnyh_peregovorov.guard_knowledge_publication_update()
returns trigger
language plpgsql
set search_path = pg_catalog
as $function$
...
before update on shablon_analiz_telefonnyh_peregovorov.knowledge_publications
for each row execute function shablon_analiz_telefonnyh_peregovorov.guard_knowledge_publication_update();

-- Validate exact manifest at activation.
create function shablon_analiz_telefonnyh_peregovorov.validate_knowledge_publication_activation()
returns trigger
language plpgsql
set search_path = pg_catalog, shablon_analiz_telefonnyh_peregovorov
as $function$
begin
  ...
  then
    ...
    if not exists (
      select 1
      from shablon_analiz_telefonnyh_peregovorov.knowledge_publication_documents
    ...
    if not exists (
      select 1
      from shablon_analiz_telefonnyh_peregovorov.knowledge_publication_fragments
      where publication_id = new.publication_id
    ) then
    ...
    if exists (
      select 1
      from shablon_analiz_telefonnyh_peregovorov.knowledge_publication_documents pd
    ...
    if exists (
      ...
      where pd.publication_id = new.publication_id
        and not exists (
          select 1
          from shablon_analiz_telefonnyh_peregovorov.knowledge_publication_fragments pf
    ...
    if exists (
      select 1
      from shablon_analiz_telefonnyh_peregovorov.knowledge_publication_fragments pf
      left join shablon_analiz_telefonnyh_peregovorov.knowledge_embeddings e
    ...
    if exists (
      ...
      join shablon_analiz_telefonnyh_peregovorov.knowledge_embeddings e
        on e.embedding_id = pf.embedding_id
      where pf.publication_id = new.publication_id
        and (
    ...
    if exists (
      ...
      where pf.publication_id = new.publication_id
        and not exists (
          select 1
          from shablon_analiz_telefonnyh_peregovorov.knowledge_publication_fragment_products pp
...
before insert or update on shablon_analiz_telefonnyh_peregovorov.knowledge_publications
for each row execute function shablon_analiz_telefonnyh_peregovorov.validate_knowledge_publication_activation();

-- Document/fragment/embedding semantic immutability once ready/published.
create function shablon_analiz_telefonnyh_peregovorov.guard_knowledge_document_update()
returns trigger
language plpgsql
set search_path = pg_catalog, shablon_analiz_telefonnyh_peregovorov
as $function$
...
begin
  ...
  select exists (
    select 1
    from shablon_analiz_telefonnyh_peregovorov.knowledge_publication_documents pd
    join shablon_analiz_telefonnyh_peregovorov.knowledge_publications p
...
before update on shablon_analiz_telefonnyh_peregovorov.knowledge_document_versions
for each row execute function shablon_analiz_telefonnyh_peregovorov.guard_knowledge_document_update();

create function shablon_analiz_telefonnyh_peregovorov.guard_knowledge_fragment_update()
returns trigger
language plpgsql
set search_path = pg_catalog, shablon_analiz_telefonnyh_peregovorov
as $function$
...
before update on shablon_analiz_telefonnyh_peregovorov.knowledge_fragments
for each row execute function shablon_analiz_telefonnyh_peregovorov.guard_knowledge_fragment_update();

create function shablon_analiz_telefonnyh_peregovorov.guard_knowledge_embedding_update()
returns trigger
language plpgsql
set search_path = pg_catalog, shablon_analiz_telefonnyh_peregovorov
as $function$
...
before update on shablon_analiz_telefonnyh_peregovorov.knowledge_embeddings
for each row execute function shablon_analiz_telefonnyh_peregovorov.guard_knowledge_embedding_update();
...
create view shablon_analiz_telefonnyh_peregovorov.v_runtime_knowledge_fragments as
select
  p.publication_id,
  ...
  e.vector_data
from shablon_analiz_telefonnyh_peregovorov.knowledge_publications p
join shablon_analiz_telefonnyh_peregovorov.knowledge_publication_fragments pf
  on pf.publication_id = p.publication_id
join shablon_analiz_telefonnyh_peregovorov.knowledge_publication_fragment_products pp
...
comment on view shablon_analiz_telefonnyh_peregovorov.v_runtime_knowledge_fragments is
