...
-- TEST/LOCAL ONLY. Depends on DB-01, DB-02 and DB-03.
--
-- Core invariant:
...
begin
  if not exists (
    ...
    raise exception 'DB-04 requires schema atp_test';
  end if;
...
create type atp_test.analysis_state as enum (
  'candidate',
  'validated',
  'current',
  'superseded',
...
create type atp_test.analysis_claim_type as enum (
  'criterion_score',
  'stage_result',
  'observation',
  'ai_outcome'
...
create type atp_test.evidence_requirement as enum (
  'none',
  'presence',
  'absence_check',
  'knowledge',
...
create type atp_test.evidence_type as enum (
  'presence',
  'absence_check',
  'knowledge',
  'composite'
...
create type atp_test.evidence_integrity as enum (
  'pending',
  'verified',
  'invalid',
...
create type atp_test.evidence_coverage as enum (
  'pending',
  'complete',
  'partial',
  'not_applicable'
...
create type atp_test.evidence_rule_kind as enum (
  'criterion',
  'stage',
  'analysis'
);

create type atp_test.observation_type as enum (
  'error',
  'strength',
  'warning'
);

create type atp_test.absence_scope_kind as enum (
  'whole_conversation',
  'stage',
  'interval'
);
...
create table atp_test.analysis_versions (
  ...
  prompt_version_id uuid not null references atp_test.prompt_versions(prompt_version_id) on delete restrict,
  methodology_version_id uuid not null references atp_test.methodology_versions(methodology_version_id) on delete restrict,
  knowledge_publication_id uuid not null references atp_test.knowledge_publications(publication_id) on delete restrict,
  ...
  reliability atp_test.processing_reliability not null,
  core_validation_status atp_test.validation_status not null default 'pending',
  evidence_gate_status atp_test.validation_status not null default 'pending',
  analysis_state atp_test.analysis_state not null default 'candidate',
  ...
  constraint analysis_versions_predecessor_same_family
    ...
    references atp_test.analysis_versions(analysis_id, call_id, family_ref)
    on delete restrict,
  ...
  constraint analysis_versions_pseudo_matches_raw
    ...
    references atp_test.pseudonymized_transcripts(
      pseudonymized_transcript_id,
      raw_transcript_id,
      call_id
    )
  ...
  constraint analysis_versions_privacy_matches_pseudo
    ...
    references atp_test.privacy_packages(
      privacy_package_id,
      pseudonymized_transcript_id,
      call_id
    )
  ...
  constraint analysis_versions_quality_matches_inputs
    ...
    references atp_test.processing_quality(
      quality_id,
      raw_transcript_id,
      role_assignment_version_id,
      call_id
...
comment on table atp_test.analysis_versions is
...
create unique index uq_analysis_versions_family_version
  on atp_test.analysis_versions (call_id, family_ref, version_no);

create unique index uq_analysis_versions_one_current
  on atp_test.analysis_versions (call_id, family_ref)
...
create table atp_test.analysis_knowledge_inputs (
  ...
  constraint analysis_knowledge_inputs_analysis_publication
    ...
    references atp_test.analysis_versions(analysis_id, knowledge_publication_id)
    on delete restrict,
  ...
  constraint analysis_knowledge_inputs_product_scope
    ...
    references atp_test.knowledge_publication_fragment_products(
      publication_id,
      fragment_id,
      product_code
    )
...
comment on table atp_test.analysis_knowledge_inputs is
...
create table atp_test.analysis_claims (
  claim_id uuid primary key default gen_random_uuid(),
  analysis_id uuid not null references atp_test.analysis_versions(analysis_id) on delete restrict,
  claim_type atp_test.analysis_claim_type not null,
  claim_key text not null,
  evidence_requirement atp_test.evidence_requirement not null,
  created_at timestamptz not null default now(),
...
comment on table atp_test.analysis_claims is
...
create table atp_test.criterion_scores (
  criterion_score_id uuid primary key,
  analysis_id uuid not null,
  claim_type atp_test.analysis_claim_type not null default 'criterion_score',
  ...
  constraint criterion_scores_claim_fk
    ...
    references atp_test.analysis_claims(claim_id, analysis_id, claim_type)
    on delete restrict,
  constraint criterion_scores_analysis_methodology
    foreign key (analysis_id, methodology_version_id)
    references atp_test.analysis_versions(analysis_id, methodology_version_id)
    on delete restrict,
  constraint criterion_scores_methodology_criterion
    foreign key (methodology_version_id, criterion_code)
    references atp_test.methodology_criteria(methodology_version_id, criterion_code)
    on delete restrict,
...
-- Stage target.
create table atp_test.stage_results (
  stage_result_id uuid primary key,
  analysis_id uuid not null,
  claim_type atp_test.analysis_claim_type not null default 'stage_result',
  methodology_version_id uuid not null,
  ...
  constraint stage_results_methodology_stage
    ...
    references atp_test.methodology_stages(methodology_version_id, stage_code)
    on delete restrict,
...
-- Observation target.
create table atp_test.analysis_observations (
  observation_id uuid primary key,
  analysis_id uuid not null,
  claim_type atp_test.analysis_claim_type not null default 'observation',
  methodology_version_id uuid not null,
  observation_code text not null,
  observation_type atp_test.observation_type not null,
...
create table atp_test.ai_inferred_outcomes (
  ai_outcome_id uuid primary key,
  analysis_id uuid not null,
  claim_type atp_test.analysis_claim_type not null default 'ai_outcome',
...
create table atp_test.evidence_sets (
  evidence_id uuid primary key default gen_random_uuid(),
  analysis_id uuid not null,
  call_id uuid not null,
  claim_id uuid not null,
  evidence_type atp_test.evidence_type not null,
  methodology_version_id uuid not null,
  rule_kind atp_test.evidence_rule_kind not null,
  criterion_code text,
  stage_code text,
  analysis_rule_code text,
  processing_quality_id uuid not null,
  reference_integrity atp_test.evidence_integrity not null default 'pending',
  coverage_status atp_test.evidence_coverage not null default 'pending',
  ...
  constraint evidence_sets_quality_fk
    ...
    references atp_test.analysis_versions(analysis_id, processing_quality_id)
    on delete restrict,
...
comment on table atp_test.evidence_sets is
...
create index ix_evidence_sets_claim
  on atp_test.evidence_sets (claim_id, created_at);

-- Conversation refs: only exact safe segments that were in the pinned privacy package.
create table atp_test.evidence_conversation_refs (
  ...
  constraint evidence_conversation_refs_analysis_package_fk
    ...
    references atp_test.analysis_versions(analysis_id, privacy_package_id)
    on delete restrict,
  constraint evidence_conversation_refs_package_segment_fk
    foreign key (privacy_package_id, pseudonymized_segment_id)
    references atp_test.privacy_package_segments(
      privacy_package_id,
...
create table atp_test.evidence_knowledge_refs (
  ...
  constraint evidence_knowledge_refs_analysis_input_fk
    ...
    references atp_test.analysis_knowledge_inputs(analysis_id, fragment_id)
    on delete restrict,
...
create table atp_test.evidence_absence_checks (
  evidence_id uuid primary key,
  analysis_id uuid not null,
  privacy_package_id uuid not null,
  methodology_version_id uuid not null,
  processing_quality_id uuid not null,
  scope_kind atp_test.absence_scope_kind not null,
  stage_code text,
  start_ms bigint,
  end_ms bigint,
...
alter table atp_test.analysis_versions
...
create function atp_test.guard_analysis_child_mutation()
returns trigger
language plpgsql
...
declare
  v_analysis_id uuid;
  v_state atp_test.analysis_state;
begin
...
before insert or update or delete on atp_test.analysis_knowledge_inputs
for each row execute function atp_test.guard_analysis_child_mutation();

create trigger trg_analysis_claims_guard
before insert or update or delete on atp_test.analysis_claims
for each row execute function atp_test.guard_analysis_child_mutation();

create trigger trg_criterion_scores_guard
before insert or update or delete on atp_test.criterion_scores
for each row execute function atp_test.guard_analysis_child_mutation();

create trigger trg_stage_results_guard
before insert or update or delete on atp_test.stage_results
for each row execute function atp_test.guard_analysis_child_mutation();

create trigger trg_analysis_observations_guard
before insert or update or delete on atp_test.analysis_observations
for each row execute function atp_test.guard_analysis_child_mutation();

create trigger trg_ai_inferred_outcomes_guard
before insert or update or delete on atp_test.ai_inferred_outcomes
for each row execute function atp_test.guard_analysis_child_mutation();

-- Criterion score must reproduce the pinned methodology criterion.
create function atp_test.validate_criterion_score()
returns trigger
language plpgsql
set search_path = pg_catalog, atp_test
as $function$
...
begin
  ...
  from atp_test.methodology_criteria
...
before insert or update on atp_test.criterion_scores
for each row execute function atp_test.validate_criterion_score();

-- Stage flags/order must reproduce the pinned methodology stage.
create function atp_test.validate_stage_result()
returns trigger
language plpgsql
set search_path = pg_catalog, atp_test
as $function$
...
begin
  ...
  from atp_test.methodology_stages
  where methodology_version_id = new.methodology_version_id
...
before insert or update on atp_test.stage_results
for each row execute function atp_test.validate_stage_result();

-- Evidence set rule must match its typed target.
create function atp_test.validate_evidence_target_rule()
returns trigger
language plpgsql
set search_path = pg_catalog, atp_test
as $function$
declare
  v_claim_type atp_test.analysis_claim_type;
  v_code text;
...
begin
  ...
  if v_claim_type = 'criterion_score' then
    ...
    into v_code
    from atp_test.criterion_scores
    ...
    from atp_test.analysis_observations
...
before insert or update on atp_test.evidence_sets
for each row execute function atp_test.validate_evidence_target_rule();
...
create function atp_test.guard_evidence_set_mutation()
returns trigger
language plpgsql
set search_path = pg_catalog, atp_test
as $function$
...
before insert or update or delete on atp_test.evidence_sets
for each row execute function atp_test.guard_evidence_set_mutation();

-- Child evidence refs freeze as soon as their evidence set is no longer pending.
create function atp_test.guard_evidence_ref_mutation()
returns trigger
language plpgsql
set search_path = pg_catalog, atp_test
as $function$
declare
  v_evidence_id uuid;
  v_integrity atp_test.evidence_integrity;
  v_analysis_state atp_test.analysis_state;
begin
  if tg_op = 'DELETE' then
...
before insert or update or delete on atp_test.evidence_conversation_refs
for each row execute function atp_test.guard_evidence_ref_mutation();

create trigger trg_evidence_knowledge_refs_guard
before insert or update or delete on atp_test.evidence_knowledge_refs
for each row execute function atp_test.guard_evidence_ref_mutation();

create trigger trg_evidence_absence_checks_guard
before insert or update or delete on atp_test.evidence_absence_checks
for each row execute function atp_test.guard_evidence_ref_mutation();

-- Conversation ref must match exact safe segment timestamps/role/quote.
create function atp_test.validate_evidence_conversation_ref()
returns trigger
language plpgsql
set search_path = pg_catalog, atp_test
as $function$
...
before insert or update on atp_test.evidence_conversation_refs
for each row execute function atp_test.validate_evidence_conversation_ref();

-- Knowledge excerpt must come from exact analysis input fragment.
create function atp_test.validate_evidence_knowledge_ref()
returns trigger
language plpgsql
set search_path = pg_catalog, atp_test
as $function$
...
begin
  ...
  into v_text
  from atp_test.analysis_knowledge_inputs aki
...
before insert or update on atp_test.evidence_knowledge_refs
for each row execute function atp_test.validate_evidence_knowledge_ref();

-- Absence evidence must use evidence_type=absence_check and exact pinned quality.
create function atp_test.validate_evidence_absence_check()
returns trigger
language plpgsql
set search_path = pg_catalog, atp_test
as $function$
declare
  v_type atp_test.evidence_type;
  v_reliability atp_test.processing_reliability;
begin
  select evidence_type
  into v_type
  from atp_test.evidence_sets
  ...
  into v_reliability
  from atp_test.processing_quality
...
before insert or update on atp_test.evidence_absence_checks
for each row execute function atp_test.validate_evidence_absence_check();

-- Overall score = weighted average of applicable criterion scores.
create function atp_test.calculate_analysis_overall_score(p_analysis_id uuid)
returns numeric
language sql
stable
set search_path = pg_catalog, atp_test
...
create function atp_test.validate_analysis_evidence_gate(p_analysis_id uuid)
returns void
language plpgsql
set search_path = pg_catalog, atp_test
as $function$
...
begin
  ...
  from atp_test.analysis_claims c
  where c.analysis_id = p_analysis_id
    and (
      (
        ...
        and not exists (
          select 1
          from atp_test.criterion_scores x
      ...
      (
        ...
        and not exists (
          select 1
          from atp_test.analysis_observations x
      ...
      (
        ...
        and not exists (
          select 1
          from atp_test.ai_inferred_outcomes x
  ...
  where e.analysis_id = p_analysis_id
    ...
    and (
      (
        ...
        and not exists (
          select 1
          from atp_test.evidence_conversation_refs c
      ...
      (
        ...
        and not exists (
          select 1
          from atp_test.evidence_knowledge_refs k
      ...
      (
        ...
        and not exists (
          select 1
          from atp_test.evidence_absence_checks a
...
create function atp_test.guard_analysis_version_update()
returns trigger
language plpgsql
set search_path = pg_catalog, atp_test
as $function$
declare
  ...
  v_privacy_status atp_test.privacy_status;
  v_prompt_state atp_test.config_version_state;
  v_methodology_state atp_test.config_version_state;
  v_publication_state atp_test.knowledge_publication_state;
...
begin
  ...
  then
    ...
    perform atp_test.validate_analysis_evidence_gate(new.analysis_id);

    v_calculated_score := atp_test.calculate_analysis_overall_score(new.analysis_id);
...
before update on atp_test.analysis_versions
for each row execute function atp_test.guard_analysis_version_update();
